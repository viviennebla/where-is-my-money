...
export default function AccountsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: { name: string; account_type: AccountType; currency: string; initial_balance: string }) =>
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...data }: { id: string; name?: string; account_type?: AccountType; is_active?: boolean; initial_balance?: string }) =>
  ...
  function openEdit(acct: Account) {
    ...
    setInitialBalance(acct.initial_balance)
    setShowForm(true)
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const data = { name, account_type: accountType, currency, initial_balance: initialBalance }
    if (editing) {
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {accounts?.map((acct) => (
          <div
            key={acct.id}
            className={`p-4 rounded-xl border ${acct.is_active ? 'bg-white border-gray-200' : 'bg-gray-50 border-gray-100 opacity-60'}`}
          >
            ...
            <div className="space-y-1">
              ...
              <div className="flex justify-between text-xs text-gray-400">
                <span>初始余额</span>
                <span>{acct.initial_balance} {acct.currency}</span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          ...
          >
            ...
            <div className="flex gap-3 mb-3">
              ...
              <label className="flex-1">
                <span className="text-sm text-gray-600">初始余额</span>
                <input
                  type="text"
